...
function setConn(on){
  const b=document.getElementById('cbadge'), t=document.getElementById('ctxt');
  b.className='conn-badge'+(on?' on':''); t.textContent=on?'ONLINE':'OFFLINE';
}

// ── HANDLER DE MENSAGENS ─────────────────────────────────────────
function handle(m){
  if(m.tipo==='log'){ addLog(m.msg, m.nivel||'data'); }
  else if(m.tipo==='status'){
    const map={agente_online:'idle',agente_offline:'idle',subida:'subida',
               descida:'descida',aguardando:'idle',concluido:'concluido',erro:'erro'};
    setFase(map[m.fase]||m.fase||'idle');
    if(m.agente_ok!==undefined) setHW(m.agente_ok?'REAL':'SEM AGENTE');
...
function setHW(modo){
  ...
  el.textContent=modo; el.className='v '+(modo==='REAL'?'v-green':'v-amber');
}
function setPref(p){
  S.pref=p;
  document.getElementById('h-pref').textContent=p.toFixed(2)+' hPa';
  document.getElementById('h-pref').className='v v-teal';
}
...
function updVel(){ document.getElementById('i-vel').textContent=(S.apAlvo/S.tSubida).toFixed(1)+' m/s'; }
...
function setFase(f){
  S.fase=f; const cfg=FCFG[f]||FCFG.idle;
  document.getElementById('fbadge').className='fbadge '+cfg.c;
  document.getElementById('ftxt').textContent=cfg.t;
  const btn=document.getElementById('btn-go');
  if(['idle','concluido','erro'].includes(f)){
    btn.disabled=false; btn.querySelector('span').textContent='INICIAR ENSAIO';
  } else {
    btn.disabled=true;  btn.querySelector('span').textContent='ENSAIO ATIVO';
  }
  atualizarDiagrama(f);
}
...
function onTelem(m){
  ...
  const elAltSys=document.getElementById('sys-alt-txt');
  if(elAltSys) elAltSys.innerHTML=`${Math.round(alt)} <span style="font-size:10px;color:var(--text2);">m</span>`;
...
function cmdIniciar(){
  ...
  if(!nomeEnsaio){
    ...
    toast('Nome do ensaio é obrigatório.','er'); return;
  }
  ...
  S.hT=[];S.hA=[];S.hV=[];S.hP=[];S.altMax=0;S.alt=0;
  S.t=0;S.tPrev=0;S.altPrev=0;S.presMin=9999;S.presMax=0;S.lastResult=null;

  ['real-line','real-fill','vel-line','vel-fill','pres-line','pres-fill'].forEach(id=>{
    const el=document.getElementById(id); if(el) el.setAttribute('points','');
  });
  ...
  const nEl=document.getElementById('inp-nome'); if(nEl) nEl.style.borderColor='';

  drawRef(); setFase('calibra');
  addLog('Ensaio iniciado  apogeu='+S.apAlvo+'m  t_sub='+S.tSubida+'s  pot='+S.potPct+'%  PWM='+S.pwm,'info');
  wsSend({cmd:'iniciar',apogeu:S.apAlvo,dur:S.tSubida,potencia_pct:S.potPct,pwm:S.pwm,
...
function exportCSV(){
  ...
  const vals=[`"${dt}"`,m.alt_alvo,m.potencia_pct,m.pwm,m.alt_max.toFixed(2),
              ...
              m.pref?m.pref.toFixed(2):'',S.presMin<9990?S.presMin.toFixed(2):''].join(',');
